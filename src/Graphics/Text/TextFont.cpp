...
void TextFont::DrawText(const std::wstring& text, float size, const glm::vec4& color, float x,
                        float y, const glm::mat4& projection) {
    ...
    for (uint32_t i = 0, j = 0; i < text.size(); i++) {
        ...
        sprite->Translate(
            (glyphs[j].xAdvance - glyphs[j].xOffset - glyphs[j].width * 0.5f) * scale.x,
...
TextFont::TextFont(const char* filePath) { LoadFont(filePath); }
...
void TextFont::ShowInfo() {
    ...
    for (unsigned int i = 0; i < glyphs.size(); i++) {
        printf("\nGlyph %d id:%d, x:%d, y:%d, w:%d, h:%d", i, glyphs[i].id, glyphs[i].x,
               glyphs[i].y, glyphs[i].width, glyphs[i].height);
    }
}

void TextFont::AddParamValue(const glm::vec4& delta) { sprite->AddParams(delta); }

void TextFont::SetParamValue(const glm::vec4& params) { sprite->SetParams(params); }
...
std::string TextFont::GetFilePath() { return filePath; }
