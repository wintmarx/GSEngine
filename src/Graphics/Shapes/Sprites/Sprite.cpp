...
constexpr float defaultVbo[] = {0.5f,  0.5f,  0.f, 1.f, 1.f, 0.5f,  -0.5f, 0.f, 1.f, 0.f,
                                -0.5f, -0.5f, 0.f, 0.f, 0.f, -0.5f, 0.5f,  0.f, 0.f, 1.f};
...
void Sprite::InitializeSprite() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertexCount * (dim + 2), defaultVbo,
                 GL_STATIC_DRAW);

    glVertexAttribPointer(0, dim, GL_FLOAT, GL_FALSE, sizeof(float) * (dim + 2), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(float) * (dim + 2),
                          (void*)(sizeof(float) * dim));
    glEnableVertexAttribArray(1);

    glGenBuffers(1, &indexBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexCount * sizeof(unsigned int), defaultIndices,
                 GL_STATIC_DRAW);
...
void Sprite::InvalidateModelMatrix() { model = tM * rM * sM; }
...
void Sprite::SetTexture(const char* filePath, bool isStdFolder) {
    ...
    SetSize(texturesController->GetTextureWidth(textureId),
            texturesController->GetTextureHeight(textureId));
}
...
void Sprite::SetColor(const glm::vec4& color) { this->color = color; }
